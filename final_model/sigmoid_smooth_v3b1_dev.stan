...
transformed data{
  // real i_std = sqrt( 1.0 / g / (J_beta + 1));
  // real r_thr = 1.0 / k * log( 1.0 / r_barrier - 1);
}
...
parameters {
  ...
  real pe_lpa[H];
  // real<lower=0> sigma_pe_lpa;
  
  vector[J_r] theta_lower;
  vector[J_r] theta_upper;
  real<lower=0> sigma_lower;
  real<lower=0> sigma_upper;
  
  vector<upper=0>[H] r_lower;
  vector<lower=0>[H] r_upper;
}
...
transformed parameters {
  ...
  for (n in 1:N) {
    int hh = h_id[n];
    row_vector[M] phi_1 = exp( -log1p_exp( -k * (r_lower[hh] - ps[n]))); // Check unnecessary
    row_vector[M] phi_2 = exp( -log1p_exp( -k * (ps[n] - r_upper[hh]))); // Check unnecessary
...
model {
  ...
  // theta_lower ~ normal(0, 0.1 * scf);
  // theta_upper ~ normal(0, 0.1 * scf);
  theta_lower ~ std_normal();
  theta_upper ~ std_normal();
  ...
  // sigma_lower ~ normal(0, 0.1 * scf);
  // sigma_upper ~ normal(0, 0.1 * scf);
  sigma_lower ~ gamma( 2, 0.1);
  sigma_upper ~ gamma( 2, 0.1);
  
  r_lower ~ normal( z_r * theta_lower, sigma_lower);
  r_upper ~ normal( z_r * theta_upper, sigma_upper);
